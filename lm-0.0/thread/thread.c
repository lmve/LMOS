/* 
 * thread.c        thread/thread.c
 * (c)  2023    lucky_ma
 * Create at 12.27
 * Changed at 12.28
 * Changed at 12.29 Add idle thread
 * Changed at 12.30 Add fork_pid
 *                  Resived thread_init
 *                  Add stdio.h fs.h file.h
 * */
...
/* 初始化线程环境 revised 12.29 Add idle*/
void thread_init(void)
{
    ...
    lock_init(&pid_lock);
    /* 创建第一个用户进程：init Add at 12.30*/
    process_execute(init, "init");          /* 这是第一个进程，pid 为 1*/
    /* 将当前main函数创建为线程 */
    make_main_thread();
    /* 创建idle线程 */
