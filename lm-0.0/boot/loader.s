;    loader.s (chapter 5, Come into kernel)       boot/loader.s
;    (c)  2023   lucky_ma  

; First use check the memory size
; use BIOS interrupt get information
...
p_mode_start:
    ...
    mov gs,ax
	                                                    ...
	                                                    ; ----------------- 加载 kernel ------------
	mov eax, KERNEL_START_SECTOR                        ; kernel.bin 所在扇区
	mov ebx, KERNEL_BIN_BASE_ADDR                       ; 从磁盘读出后，写入 ebx 指定的地址
	mov ecx, 200                                        ; 读入的扇区数
	call rd_disk_m_32
