/*
 * memory.c        kernel/memory.c
 * (c)  2023    lucky_ma
 * 12.26 Create memory.c
 * 12.28 Changed
 * 12.30 Add get_a_page_without_opvaddrbitmap
 */
...
struct pool {
   ...
   struct lock lock;		 // 申请内存时互斥  Add 12.28						
};
...
void* get_kernel_pages(uint32_t pg_cnt) {
    lock_acquire(&kernel_pool.lock);  // 12.28 add
   	void* vaddr =  malloc_page(PF_KERNEL, pg_cnt);
   	if (vaddr != NULL) {	   // 若分配的地址不为空,将页框清0后返回
      	memset(vaddr, 0, pg_cnt * PG_SIZE);
   	}
	lock_release(&kernel_pool.lock); //12.28 add
   	return vaddr;
}
